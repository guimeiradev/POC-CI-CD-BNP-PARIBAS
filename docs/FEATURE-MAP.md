...
`CicdPoc.Api` process is up and responding.

**Flow:**

1. `src/CicdPoc.Api/Program.cs` — `GET /health` route handler returns `200 OK` with `{"status":"healthy"}`.
2. `src/CicdPoc.Api.Tests/HealthEndpointTests.cs` — integration test boots the app in-process via `WebApplicationFactory<Program>`, calls `GET /health`, and asserts the response is successful and contains `"healthy"`.
...
1. `Jenkinsfile` (Restore stage) — `dotnet restore src/CicdPoc.sln` restores NuGet packages for both projects.
2. `Jenkinsfile` (Database Migration stage) — waits for `sqlserver:1433`, then runs Liquibase (`execute-sql` to create `CicdPocDb`, then `update` against `db/changelog/db.changelog-master.xml`) using the `sqlserver-credentials` credential.
...
7. `Jenkinsfile` (Publish stage) — `dotnet publish src/CicdPoc.Api/CicdPoc.Api.csproj -c Release --no-build -o publish/` produces the deployable output.
8. `Jenkinsfile` (Archive stage) — zips `publish/` into `CicdPoc.Api-${ARTIFACT_VERSION}.zip` (semver: `${BASE_VERSION}-build.${BUILD_NUMBER}`, e.g. `CicdPoc.Api-1.0.0-build.42.zip`) and writes `${ARTIFACT_NAME}.sha256` via `sha256sum`.
9. `Jenkinsfile` (Upload to Nexus stage) — uploads the zip **and** its `.sha256` to `http://nexus:8081/repository/dotnet-artifacts`, authenticating with the Jenkins `nexus-credentials` credential. As of Fase 6 the upload is performed by the Shared Library step `jenkins-shared-library/vars/nexusUpload.groovy`, consumed via `@Library('cicd-shared@main')` at the top of the `Jenkinsfile` (registered as the `cicd-shared` Global Pipeline Library — see `cicd-poc/README.md` Fase 6).
...
3. `deploy/ansible/deploy.yml` (deploy) — unarchives the release, atomically repoints the `current` symlink, templates `templates/cicdpoc-api.service.j2` into `/etc/systemd/system/cicdpoc-api.service`, and restarts the `cicdpoc-api` systemd unit (`systemd_service`, `become: true`).
